...
Phoenix::Tiledmap::Tiledmap(std::string filename, System::Scene& scene)
{
	...
	Tools::logAction(imagedir);
	m_tileset.loadFromFile(imagedir);
		...
		if ((std::string)(t.first.data()) == "layer")
		{
			...
			BOOST_FOREACH(boost::property_tree::ptree::value_type &v, t.second.get_child("data"))
			{
				if (v.second.get<int>("<xmlattr>.gid") != 0)
				{
					...
					{
						b2BodyDef def;
						def.position = tools::sTOb(i * tileSize.x + tileSize.x / 2, j * tileSize.y + tileSize.y / 2);
						b2Body* body = scene.addPhysics(def);
						b2Vec2 sizeboxsim(tools::sTOb(tileSize.x/2, tileSize.y/2));
						...
						//tools::logAction<std::string>("Tilemap tile made");
						//delete body;
					}
				}
				i++;
...
void Phoenix::Tiledmap::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
